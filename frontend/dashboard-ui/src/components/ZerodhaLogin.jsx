...
export default function ZerodhaLogin() {
  ...
  const requestAuthUrl = useCallback(async () => {
    ...
    try {
      ...
      const authUrl = authData?.auth_url || null;
      setState(prev => ({
        ...prev,
        authLoading: false,
        authUrl,
  ...
  const handleLoginClick = async () => {
    ...
    const fetchedUrl = await requestAuthUrl();
    if (fetchedUrl) {
      window.open(fetchedUrl, '_blank', 'noopener');
    }
  };
      ...
      <div className="max-w-xl w-full bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-blue-200 dark:border-slate-700 p-8 space-y-6">
        ...
        <div className="space-y-3 text-sm text-gray-600 dark:text-gray-300">
          ...
          <ol className="list-decimal list-inside space-y-1">
            ...
            <li>When redirected back, the dashboard will load automatically.</li>
          </ol>
        </div>
